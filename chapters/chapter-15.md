...
1.从https://github.com/drush-ops/drush上下载Drush。在Github上Drush所在页面右侧你会看到一下Download Zip的按钮，如果你熟悉Git（见第16章），你可能想要克隆Git仓库以方便管理Drush的更新。有关克隆仓库的信息，请参见第16章。
...
要在你的当前会话中应用选项b和c，可以登出并重新登录或使用下边的命令重新加载bash配置文件：$ source .bashrc

___

注意：如果你不走这一步，你将需要（不方便的）使用完整路径在可执行文件下运行Drush命令，/pash/to/drush/drush，或导航到 /path/to/drush 然后运行 ./drush。将需要-r或-l选项（见下方的使用）。
...
status命令返回一个有关你当前Drupal8安装信息的有用的列表，另一个重要例子是使用drush pm-download命令行下载一个模块或主题：  
